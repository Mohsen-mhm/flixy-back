...
class ContentController extends Controller
{
    ...
    public function fetchMoviesList(Request $request)
    {
        ...
        $data = $result->map(function ($item) {
            ...
            $action = "<div class='text-end action'>{$movieDetail}{$edit}{$delete}</div>";
    ...
    public function addNewContent(Request $request)
    {
        ...
        } elseif ($request->has('movie_content_id')) {
            $this->saveMovieCastsActors($request->input('movie_content_id'), $content->id);
            $this->saveMovieCastsFactors($request->input('movie_content_id'), $content->id);
        }
    ...
    {
        set_time_limit(600);
        $this->saveCastsActors($tmdbContentId, $contentId);
        $this->saveCastsFactors($tmdbContentId, $contentId);
            ...
            foreach ($seasonsResponse['seasons'] as $season) {
                ...
                if (array_key_exists('episodes', $episodesResponse)) {
                    foreach ($episodesResponse['episodes'] as $episode) {
                        if (array_key_exists('still_path', $episode) && $episode['still_path']) {
    ...
    public function fetchCastList(Request $request)
    {
        ...
        $data = $result->map(function ($item) {
            $imgSrc = $item->actor->profile_image ?: asset('assets/img/default.png');
    ...
    public function fetchSeriesList(Request $request)
    {
        ...
        $data = $result->map(function ($item) {
            ...
            $seriesDetail = "<a href='series/{$item->id}' class='btn btn-info me-2 shadow-none text-white' style='white-space: nowrap;'>" . __('seriesDetail') . "</a>";
            ...
            $action = "<div class='text-end action'>{$seriesDetail}{$edit}{$delete}</div>";
     ...
     * @return void
     */
    public function saveCastsActors($tmdbContentId, $contentId): void
    {
    ...
    public function saveMovieCastsActors($tmdbContentId, $contentId)
    {
        $creditsResponse = Http::get("https://api.themoviedb.org/3/movie/{$tmdbContentId}/credits", [
            'api_key' => env('TMDB_API_KEY'),
            'language' => 'en-US',
        ])->json();
            ...
            foreach ($creditsResponse['cast'] as $cast) {
                if ($cast['known_for_department'] == 'Acting') {
                    ...
                    if (!$contentCast) {
                        ...
                        $contentCast->content_id = $contentId;
                        $contentCast->actor_id = $actor->id;
                        $contentCast->character_name = array_key_exists('character', $cast) && $cast['character'] ? $cast['character'] : '';
                        $contentCast->save();
                    }
                    $counter++;
     ...
     * @return void
     */
    public function saveCastsFactors($tmdbContentId, $contentId): void
    {
        $creditsResponse = Http::get("https://api.themoviedb.org/3/tv/{$tmdbContentId}/aggregate_credits", [
            'api_key' => env('TMDB_API_KEY'),
            'language' => 'en-US',
    ...
    public function saveMovieCastsFactors($tmdbContentId, $contentId)
    {
        $creditsResponse = Http::get("https://api.themoviedb.org/3/movie/{$tmdbContentId}/credits", [
            'api_key' => env('TMDB_API_KEY'),
            'language' => 'en-US',
                ...
                if (in_array($cast['known_for_department'], ['Directing', 'Production', 'Writing'])) {
                    if (in_array($cast['job'], ['Director', 'Producer', 'Writer'])) {
                        ...
                        if (!$contentCast) {
                            ...
                            $contentCast->content_id = $contentId;
                            $contentCast->actor_id = $actor->id;
                            $contentCast->character_name = array_key_exists('job', $cast) && $cast['job'] ? $cast['job'] : 'not defined';
                            $contentCast->save();
                        }
                        $counter++;
