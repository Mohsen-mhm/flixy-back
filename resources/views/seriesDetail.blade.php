...
@section('content')
        ...
        <section class="detail_list">
            <div class="card" id="detail_content">
                <div class="card-header border-0 d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center">
                        <img
                            src='{{ $content->horizontal_poster ? $content->horizontal_poster : asset("assets/img/default.png") }}'
                            alt="" class="rounded border-muted object-fit-cover" data-fancybox
                            style="background: rgb(118 118 128 / 12%);" width="100px" height="60px" class="">
                        <h4 class="fw-normal ms-3 mb-0"> {{ $content->title }} </h4>
                    </div>
                    <div class="d-flex align-items-center justify-content-between">
                        <a class='me-2 btn btn-success px-4 py-2 text-white editContentModal' rel='{{$content->id}}'
                           data-type='{{$content->type}}' data-title='{{$content->title}}'
                           data-description='{{$content->description}}' data-duration='{{$content->duration}}'
                           data-release_year='{{$content->release_year}}' data-ratings='{{$content->ratings}}'
                           data-language_id='{{$content->language_id}}' data-genre_ids='{{$content->genre_ids}}'
                           data-trailer_url='{{$content->trailer_url}}' data-vposter='{{ $content->vertical_poster }}'
                           data-hposter='{{ $content->horizontal_poster }}'>{{ __('edit') }}</a>
                        <a class='btn btn-danger px-4 py-2 text-white deleteContent'
                           rel='{{$content->id}}'>{{ __('delete') }}</a>
                    </div>
                </div>
            </div>
            ...
            <nav class="card-tab mb-4 d-flex align-items-center justify-content-between">
                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                    <button class="nav-link active" id="nav-season-tab" data-bs-toggle="tab"
                            data-bs-target="#nav-season" type="button" role="tab" aria-controls="nav-season"
                            aria-selected="false">
                        {{ __('season') }}
                    </button>
                    <button class="nav-link" id="nav-cast-tab" data-bs-toggle="tab" data-bs-target="#nav-cast"
                            type="button" role="tab" aria-controls="nav-cast" aria-selected="false">
                        {{ __('cast') }}
                    </button>
                </div>
                <button type="button" class="btn btn-primary text-light px-4" data-bs-toggle="modal"
            ...
            <div class="tab-content" id="nav-tabContent">
                <div class="tab-pane show active" id="nav-season" role="tabpanel" aria-labelledby="nav-season-tab"
                     tabindex="0">
                    <div id="is_season">
                            ...
                            <div class="col-md-4 mb-4">
                                <div class="selectSeason d-flex">
                                    <div class="form-group w-100 m-0 me-2 season-select">
                                        <select name="season_list" id="season_list" class="form-control selectric"
                                                required="">
                                            <option value="0" disabled class="d-none"> {{ __('seasonList')}}</option>
                                            @foreach ($seasons as $season)
                                                <option value="{{ $season->id }}"
                                                        data-trailerurl="{{ $season->trailer_url }}"
                                    ...
                                    <div id="season_action" class="w-auto">
                                        <div class="text-end d-flex">
                                            <div class="me-2 btn btn-success px-4 text-white edit" id="edit_season"
                                                 rel='{{ $seasons->first() ? $seasons->first()->id : "" }}'
                                                 data-title='{{ $seasons->first() ? $seasons->first()->title : ""  }}'
                                                 data-trailerurl='{{ $seasons->first() ? $seasons->first()->trailer_url : ""  }}'>
                                                {{ __('edit') }}
                                            </div>
                                            <div class="btn btn-danger px-4 text-white delete" id="delete_season"
                                                 rel='{{ $seasons->first() ? $seasons->first()->id : "" }}'>
                                                {{ __('delete') }}
                                            </div>
                                        </div>
                            ...
                            <div class="card-header d-flex align-items-center justify-content-between">
                                <div class="d-flex align-items-center justify-content-between">
                                    <h4 class="fw-normal m-0">
                                        <span
                                            id="seasonBadge"> {{ $seasons->first() ? $seasons->first()->title : ""  }} </span>
                                    </h4>
                                    <a href="https://youtu.be/{{ $seasons->first() ? $seasons->first()->trailer_url : ''  }}"
                                       target="_blank" rel="noopener noreferrer" id="youtube_id"
                                       class="bg-white theme-color px-3 fw-semibold rounded"> {{ __('trailer') }} </a>
                                </div>
                                <button type="button" class="btn btn-primary text-light px-4" data-bs-toggle="modal"
                ...
                <form id="addSeasonForm" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group m-0">
                            ...
                            <input name="trailer_url" type="text" class="form-control" id="trailer_url"
                                   aria-describedby="trailer_url" required>
                        </div>
                    </div>
    ...
    <!-- Edit Season Modal -->
    <div class="modal fade" id="editSeasonModal" class="editSeasonModal" tabindex="-1"
         aria-labelledby="editSeasonModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input name="title" type="text" class="form-control" id="edit_title"
                                   aria-describedby="title" required value="">
                        </div>
                        <div class="form-group m-0">
                            <label for="edit_trailer_url" class="form-label">{{ __('youtubeID') }} (GUMDnD*****)</label>
                            <input name="trailer_url" type="text" class="form-control" id="edit_trailer_url"
                                   aria-describedby="trailer_url" required value="">
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn text-light" data-bs-dismiss="modal">{{ __('close') }}</button>
                ...
                <form id="addEpisodeForm" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input name="number" type="number" class="form-control" id="number" required min="0"
                                   max="99" oninput="this.value = this.value.slice(0, 2);">
                        </div>
                        <div class="form-group">
                            <label for="title" class="form-label">{{ __('title') }}</label>
                            <input name="title" type="text" class="form-control" id="episode_title"
                        ...
                        <div class="row">
                            <div class="col-auto">
                                <div class="form-group">
                                    <label for="horizontal_poster"
                                           class="form-label">{{ __('horizontalPoster') }}</label>
                                    <div class="horizontalPosterImg position-relative">
                                        <div class="upload-options">
                                            <label for="thumbnail">
                                                <input name="thumbnail" type="file" accept=".png, .jpg, .jpeg, .webp"
                                                       onchange="loadFile(event, 'thumbnailPoster')" id="thumbnail">
                                            </label>
                                        </div>
                                        <img id="thumbnailPoster"
                                             class="custom_img img-fluid horizontal_poster modal_placeholder_image"
                                             src="{{ asset('./assets/img/placeholder-image.png')}}">
                                    </div>
                                </div>
                            </div>
                            <div class="col">
            ...
            <div class="modal-content">
                ...
                <form id="editEpisodeForm" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="number" class="form-label">{{ __('episodeNumber') }}</label>
                            <input name="number" type="number" class="form-control" id="edit_number" required min="0"
                                   max="99" oninput="this.value = this.value.slice(0, 2);">
                        </div>
                        <div class="form-group">
                            <label for="title" class="form-label">{{ __('title') }}</label>
                            <input name="title" type="text" class="form-control" id="edit_episode_title"
                                   aria-describedby="title" required>
                        </div>
                        <div class="form-group">
                            <label for="description" class="form-label">{{ __('description') }}</label>
                        ...
                        <div class="row">
                                    ...
                                    <div class="horizontalPosterImg position-relative">
                                        <div class="upload-options">
                                            <label for="edit_thumbnail">
                                                <input name="thumbnail" type="file" accept=".png, .jpg, .jpeg, .webp"
                                                       onchange="loadFile(event, 'edit_thumbnailPoster')"
                                                       id="edit_thumbnail">
                                            </label>
                                        </div>
                                        <img id="edit_thumbnailPoster"
                                             class="custom_img img-fluid horizontal_poster modal_placeholder_image"
                                             src="{{ asset('assets/img/placeholder-image.png')}}">
                                    </div>
                                </div>
                            ...
                            <div class="col">
                                <div class="form-group">
                                    <label for="duration" class="form-label">{{ __('duration') }}</label>
                                    <input name="duration" type="text" class="form-control" id="edit_duration"
                                           aria-describedby="duration" required>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <input name="character_name" type="text" class="form-control" id="character_name"
                                       aria-describedby="character_name" required>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <input name="character_name" id="edit_character_name" type="text" class="form-control"
                                       aria-describedby="character_name" required="">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row">
                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12">
                                <div class="form-group">
                                    ...
                                    <select name="type" id="edit_content_type" class="form-control selectric"
                                            required="" disabled>
                                        <option value="" disabled selected
                                                class="d-none">{{ __('selectOption') }}</option>
                                ...
                                <div class="form-group">
                                    <label for="title" class="form-label">{{ __('title') }}</label>
                                    <input name="title" type="text" class="form-control" id="edit_content_title"
                                           aria-describedby="title" required="">
                                </div>
                                <div class="form-group">
                                    <label for="description" class="form-label">{{ __('description') }}</label>
                                    <textarea name="description" id="edit_content_description" rows="4"
                                              class="form-control" required=""></textarea>
                                </div>
                                <div class="form-group">
                                    <label for="selectGenre" class="form-label">{{ __('selectGenre') }}
                                        ({{ __('multiple') }})</label>
                                    <select name="genre_ids[]" class="form-control selectric" id="edit_selectGenre"
                                            multiple="multiple" required="" tabindex="-1" aria-hidden="true">
                                ...
                                <div class="form-group m-0">
                                    ...
                                    <select name="language_id" id="edit_language_id" class="form-control selectric"
                                            required="">
                                        <option value="" disabled selected
                                                class="d-none">{{ __('selectLanguage') }}</option>
                            ...
                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12">
                                <div class="row">
                                    <div class="col">
                                        <div class="form-group">
                                            ...
                                            <input name="release_year" type="number" class="form-control"
                                                   id="edit_release_year" aria-describedby="release_year" required="">
                                        </div>
                                    </div>
                                    <div class="col">
                                        <div class="form-group">
                                            ...
                                            <div class="rating_star">
                                                ...
                                                <input name="ratings" type="number" step=any class="form-control"
                                                       id="edit_ratings" aria-describedby="rating" required="">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="d-flex">
                                    <div class="form-group m-0">
                                        <label for="vertical_poster"
                                               class="form-label">{{ __('verticalPoster') }}</label>
                                        <div class="posterImg position-relative">
                                            <div class="upload-options">
                                                <label for="edit_poster">
                                                    <input name="vertical_poster" type="file"
                                                           accept=".png, .jpg, .jpeg, .webp"
                                                           onchange="loadFile(event, 'edit_vertical_poster')"
                                                           id="edit_poster">
                                                </label>
                                            </div>
                                            <img id="edit_vertical_poster"
                                                 class="custom_img img-fluid vertical_poster modal_placeholder_image"
                                                 src="{{ asset('assets/img/placeholder-image.png')}}">
                                        </div>
                                    </div>
                                        ...
                                        <div class="horizontalPosterImg position-relative">
                                            <div class="upload-options">
                                                <label for="edit_horizontalPoster">
                                                    <input name="horizontal_poster" type="file"
                                                           accept=".png, .jpg, .jpeg, .webp"
                                                           onchange="loadFile(event, 'edit_horizontalPosterImg')"
                                                           id="edit_horizontalPoster">
                                                </label>
                                            </div>
                                            <img id="edit_horizontalPosterImg"
                                                 class="custom_img img-fluid horizontal_poster modal_placeholder_image"
                                                 src="{{ asset('assets/img/placeholder-image.png')}}">
                                        </div>
                                    </div>
